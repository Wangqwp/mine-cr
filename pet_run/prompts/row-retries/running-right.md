Create Codex pet row `running-right` for `my-dog`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A cute pet dog based on my dogs photo. Preserve silhouette, face, palette, material, proportions, markings, and props.
